...
The **Window is closed** condition passes when a targeted window is currently closed. Use it when you want to make sure the house is shut before you lock a door, arm an alarm, or turn heating back on.
...
### Automation: lock the front door at night only if all downstairs windows are closed
...
### Automation: start cooling only if the bedroom window has been closed for 15 minutes

If you use a motorized bedroom window to cool the room naturally, you might only want to restart cooling after the window has been closed for a while.
...
- **Action**: Climate: Turn on

{% details "YAML example for restarting cooling after a window stays closed" %}

{% example %}
automation: |
  alias: "Start cooling after the bedroom window stays closed"
  triggers:
